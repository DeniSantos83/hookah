...
export default function AdminPage() {
  ...
  const [fotosAprovadas, setFotosAprovadas] = useState([]);
  ...
  async function carregarPainel() {
    try {
      const { data, error } = await supabase.rpc("painel_anfitriao", {
      ...
      if (!data?.sucesso) {
        throw new Error(
          data?.mensagem || "Não foi possível carregar o painel.",
        );
      }
  ...
  async function carregarFotos() {
    try {
      const { data, error } = await supabase.rpc("fotos_pendentes", {
        p_sala_codigo: "NARGUILEAJU",
      });
      ...
      if (!data?.sucesso) {
        throw new Error(
          data?.mensagem || "Não foi possível carregar as fotos.",
        );
      }
      ...
      setErro("Não foi possível carregar as fotos pendentes.");
    }
  }
  ...
  async function carregarFotosAprovadas() {
    try {
      const { data, error } = await supabase.rpc("fotos_aprovadas_admin", {
        p_sala_codigo: "NARGUILEAJU",
      });
      ...
      if (!data?.sucesso) {
        throw new Error(
          data?.mensagem || "Não foi possível carregar as fotos aprovadas.",
        );
      }

      setFotosAprovadas(data?.fotos || []);
    } catch (error) {
      console.error("Erro ao carregar fotos aprovadas:", error);
      setErro("Não foi possível carregar as fotos aprovadas.");
    }
  }
  ...
  async function executarAcao(funcao, parametros) {
    if (processando) {
      return;
    }
    ...
    try {
      ...
      if (!data?.sucesso) {
        throw new Error(
          data?.mensagem || "Não foi possível realizar a operação.",
        );
      }

      setMensagem(data?.mensagem || "Operação realizada.");
  ...
  function alterarLimite(limite) {
    executarAcao("alterar_limite_sala", {
      p_sala_codigo: "NARGUILEAJU",
      p_limite: limite,
    });
  }
  ...
  function alterarDuracaoMaxima(segundos) {
    executarAcao("alterar_duracao_maxima_sala", {
      p_sala_codigo: "NARGUILEAJU",
  ...
  function salvarMensagemTv() {
    ...
    if (mensagemTvAtiva && !texto) {
      setErro("Digite uma mensagem antes de ativar o letreiro.");
      return;
    }

    executarAcao("alterar_mensagem_tv", {
      p_sala_codigo: "NARGUILEAJU",
  ...
  function removerMusica(id) {
    const confirmar = window.confirm("Remover esta música da fila?");

    if (!confirmar) {
      return;
    }

    executarAcao("remover_musica_fila", {
      p_sala_codigo: "NARGUILEAJU",
      p_musica_id: id,
    });
  }
  ...
  async function alterarFoto(fotoId, novoStatus) {
    if (processandoFoto) {
      return;
    }
    ...
    try {
      const { data, error } = await supabase.rpc("alterar_status_foto", {
        p_foto_id: fotoId,
        p_status: novoStatus,
      });
      ...
      if (!data?.sucesso) {
        throw new Error(data?.mensagem || "Não foi possível alterar a foto.");
      }

      if (novoStatus === "aprovada") {
        setMensagem("Foto aprovada para exibição na TV.");
      } else {
        setMensagem("Foto rejeitada.");
      }
      ...
      setErro(error.message || "Não foi possível atualizar a foto.");
    } finally {
  ...
  async function excluirFoto(foto) {
    ...
    const confirmar = window.confirm(
      "Excluir esta foto permanentemente? Ela também será removida do telão.",
    );
    ...
    setProcessandoFoto(foto.id);
    setErro("");
    setMensagem("");

    try {
      // 1. Remove o arquivo usando a API oficial do Supabase Storage.
      ...
      // 2. Depois remove o registro da tabela public.fotos.
      const { data, error } = await supabase.rpc("excluir_foto", {
        p_sala_codigo: "NARGUILEAJU",
        p_foto_id: foto.id,
      });
      ...
      if (!data?.sucesso) {
        throw new Error(data?.mensagem || "Não foi possível excluir a foto.");
      }

      setMensagem("Foto excluída permanentemente.");

      await Promise.all([carregarFotos(), carregarFotosAprovadas()]);
    } catch (error) {
  ...
  function abrirTv() {
    const urlTv = `${window.location.origin}${window.location.pathname}#/tv`;

    window.open(urlTv, "_blank", "noopener,noreferrer");
  }
  ...
  async function sair() {
    try {
      ...
      localStorage.removeItem("narguileaju_admin");
  ...
  return (
    <div className="admin-page">
      ...
      <header className="admin-header">
        <div>
          <span className="admin-brand">🐪 NARGUILEAJU</span>

          <h1>Painel do Anfitrião</h1>

          <p>Controle da música e da experiência do lounge</p>
        </div>
      ...
      <main className="admin-content">
        ...
        <section className="admin-card">
          <div className="admin-card-title">
            <div>
              ...
              <h2>Limite por pessoa</h2>
            </div>

            <strong className="current-limit">{painel?.limite ?? 0}</strong>
          </div>

          <p className="limit-description">
            Quantidade máxima de músicas que cada cliente pode manter
          ...
          <div className="limit-buttons">
            {[1, 2, 3, 4, 5].map((numero) => (
              <button
                ...
                key={numero}
                disabled={processando}
                className={painel?.limite === numero ? "limit-active" : ""}
                onClick={() => alterarLimite(numero)}
              >
                {numero}
              </button>
            ))}
          ...
          <div className="quick-limit">
            ...
            <button
              type="button"
              disabled={processando}
              onClick={() => alterarLimite(1)}
            >
              Lounge cheio
              <strong>1 música</strong>
            </button>
          </div>
        </section>
        ...
        <section className="admin-card">
          <div className="admin-card-title">
            <div>
              ...
              <h2>Duração máxima por música</h2>
            </div>

            <strong className="duration-current-limit">
              {painel?.duracao_maxima_segundos == null
                ? "∞"
                : `${Math.round(painel.duracao_maxima_segundos / 60)} min`}
            </strong>
          </div>

          <p className="limit-description">
            Vídeos acima do tempo definido ficam indisponíveis para os clientes
            na busca de músicas.
          </p>
          ...
          <div className="duration-limit-buttons">
            {[
              ...
              <button
                ...
                onClick={() => alterarDuracaoMaxima(opcao.value)}
              >
                {opcao.label}
              </button>
            ))}
        ...
        <section className="admin-card admin-tv-message-card">
          ...
          <p className="limit-description">
            A mensagem passa continuamente no rodapé do vídeo e fica atrás do
            card das fotos.
          </p>
          ...
          <textarea
            ...
            onChange={(event) => setMensagemTv(event.target.value)}
          />

          <div className="admin-tv-message-meta">
            <span>{mensagemTv.length} / 220 caracteres</span>

            <span>A alteração aparece no telão automaticamente.</span>
          </div>
          ...
          <div className="admin-tv-message-actions">
            ...
            <button
              ...
              disabled={processando}
              onClick={() => alterarStatusMensagemTv(!mensagemTvAtiva)}
            >
              {mensagemTvAtiva ? "DESATIVAR LETREIRO" : "ATIVAR LETREIRO"}
            </button>
          </div>
        </section>
        ...
        <section className="admin-card">
          <div className="admin-section-header">
            ...
            {tocando && <div className="playing-dot">▶ AO VIVO</div>}
          </div>

          {tocando ? (
            <div className="admin-now-playing">
              {tocando.thumbnail_url ? (
                <img src={tocando.thumbnail_url} alt="" />
              ) : (
                <div className="admin-thumbnail-placeholder">♫</div>
              )}

              <div className="now-playing-info">
                <strong>{tocando.titulo}</strong>

                <span>{tocando.artista}</span>

                <small>
                  Pedido por <b>{tocando.nickname || "Cliente"}</b>
                </small>
              </div>
            </div>
          ) : (
          ...
          <button
            ...
            disabled={processando || (!tocando && fila.length === 0)}
        ...
        <section className="admin-card">
          <div className="admin-section-header">
            ...
            <strong className="queue-count">{fila.length}</strong>
          </div>
          ...
          {fila.length === 0 ? (
            ...
            <div className="admin-queue">
              {fila.map((musica, index) => (
                <article className="admin-music" key={musica.id}>
                  <div className="queue-position">{index + 1}</div>

                  {musica.thumbnail_url ? (
                    <img src={musica.thumbnail_url} alt="" />
                  ) : (
                    <div className="admin-music-placeholder">♫</div>
                  )}
                  ...
                  <div className="admin-music-info">
                    ...
                    <small>
                      Pedido por <b>{musica.nickname || "Cliente"}</b>
                    </small>
                  </div>
        ...
        <section className="admin-card">
          <div className="admin-section-header">
            ...
            <strong className="queue-count">{fotos.length}</strong>
          </div>

          <p className="admin-photo-description">
            Aprove apenas fotos que podem ser exibidas publicamente na TV do
            lounge.
          </p>
          ...
          {fotos.length === 0 ? (
            ...
            <div className="admin-photo-grid">
              {fotos.map((foto) => (
                <article className="admin-photo-card" key={foto.id}>
                  <div className="admin-photo-image">
                    <img src={foto.arquivo_url} alt="Foto enviada" />
                  </div>

                  <div className="admin-photo-info">
                    <span>Enviada por</span>

                    <strong>{foto.nickname || "Cliente"}</strong>
                  </div>
                  ...
                  <div className="admin-photo-actions">
                    <button
                      ...
                      onClick={() => alterarFoto(foto.id, "aprovada")}
                    >
                      {processandoFoto === foto.id ? "..." : "✓ Aprovar"}
                    </button>
                    ...
                    <button
                      ...
                      disabled={processandoFoto !== null}
                      onClick={() => alterarFoto(foto.id, "rejeitada")}
                    >
                      × Rejeitar
                    </button>
                  </div>
        ...
        <section className="admin-card">
          <div className="admin-section-header">
            <div>
              <span>SOCIAL WALL</span>
              <h2>Fotos no telão</h2>
            </div>

            <strong className="queue-count">{fotosAprovadas.length}</strong>
          </div>

          <p className="admin-photo-description">
            Fotos aprovadas para exibição. A exclusão remove a foto
          ...
          {fotosAprovadas.length === 0 ? (
            ...
            <div className="admin-photo-grid">
              {fotosAprovadas.map((foto) => (
                <article className="admin-photo-card" key={foto.id}>
                  <div className="admin-photo-image">
                    <img src={foto.arquivo_url} alt="Foto aprovada" />
                  </div>
